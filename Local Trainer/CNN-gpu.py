...
if not os.path.exists("CNN_mnist_model.npz"):
    ...
    kernal_2 = (np.random.randn(20, 10, 3, 3) * np.sqrt(2/90)).astype(np.float32) #random init values for 20 groups of 10 3x3 kernal (filter) (small for first init values_ -(array of weights)
...
for e in range(10): #train for 5 epochs
    ...
    numpy.random.shuffle(dataset) #randomly shuffle again
    ...
    for b in range(25000//batch_size): #integer division (25000/64 is not clean)
        ...
        if b == 0:
            ...
            cost_past.append(batch_cost.get()) #append batch cost and accuracy +.get() for gpu retreival
            accuracy_past.append(batch_accuracy) #no need for .get() as it is already cpu side
        #backprop
...
#so the derivatives are added on top of eachother due to the nature of convolution
    ...
    numpy.savez( #save model after every epoch (all batches completed)
        "CNN_mnist_model.npz",
        kernal=params[0],
        kernal_bias=params[1],
        kernal_2=params[2],
